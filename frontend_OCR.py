...
if 'auto_gender' not in st.session_state: st.session_state['auto_gender'] = ""
if 'auto_address' not in st.session_state: st.session_state['auto_address'] = ""
if 'auto_age' not in st.session_state: st.session_state['auto_age'] = None
...
if not st.session_state['token']:
    ...
    with tab2:
            ...
            if st.button("Extract from ID"):
                ...
                try:
                    ...
                    with open(temp_file_path, "wb") as f:
                        f.write(uploaded_id.getvalue())

                    # 2. Run local OCR
                    with st.spinner("Running deep learning OCR model locally..."):
                        # Call the combined OCR function from ocr_processor.py
                        ocr_data = run_ocr_on_file(temp_file_path)

                    st.success("ID Extracted Successfully!")

                    # 3. Update Session State values
                    st.session_state['auto_name'] = ocr_data.get("name", "")
                    st.session_state['auto_gender'] = ocr_data.get("gender", "")
                    st.session_state['auto_address'] = ocr_data.get("address", "")
                    st.session_state['auto_age'] = ocr_data.get("age", None)

                    # Rerun Streamlit to apply new values to the form
                    st.rerun()

                except Exception as e:
                    # Display any errors during OCR processing
                    st.error(f"❌ Failed to extract info. Please check image clarity and file paths: {e}")
        ...
        # ---------------------------------------------------------------------------------
        # The single, unified form starts here. It uses st.session_state for persistent values.
        with st.form("signup_form_unified"):
            new_user = st.text_input("Choose a Username")
            new_pass = st.text_input("Choose a Password", type="password")

            # Read values from Session State first, falling back to empty string or initial value
            full_name = st.text_input("Full Name", value=st.session_state.get('auto_name', auto_name))

            role_select = st.selectbox("I am a...", ["patient", "doctor"])

            if role_select == "patient":
                gender = st.text_input("Gender", value=st.session_state.get('auto_gender', auto_gender))
                age = st.number_input("Age", min_value=0, max_value=120,
                                      value=st.session_state.get('auto_age', auto_age if auto_age else 0))
                address = st.text_area("Address", value=st.session_state.get('auto_address', auto_address))
            else:
                spec = st.text_input("Specialization")

            # Logic for registration submit button
